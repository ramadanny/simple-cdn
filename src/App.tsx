...
function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const handleRename = async (path: string, sha: string) => {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <div className="w-16 h-16 bg-zinc-50 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
            {uploading ? <Loader2 className="w-8 h-8 text-zinc-400 animate-spin" /> : <Upload className="w-8 h-8 text-zinc-400" />}
          </div>
          <h3 className="text-lg font-semibold text-zinc-900">
            {uploading ? "Uploading files..." : "Upload Assets"}
          </h3>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {filteredFiles.map((file) => (
              <FileCard key={file.sha} file={file} onDelete={handleDelete} onRename={handleRename} />
            ))}
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-zinc-50 border-b border-zinc-200">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider hidden sm:table-cell">Size</th>
                  <th className="px-6 py-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-100">
                {filteredFiles.map((file) => (
                  <FileRow key={file.sha} file={file} onDelete={handleDelete} onRename={handleRename} />
                ))}
              </tbody>
            </table>
          </div>
...
function FileCard({ file, onDelete, onRename }: FileComponentProps) {
  ...
  return (
    <motion.div 
      ...
      className="group bg-white rounded-xl border border-zinc-200 overflow-hidden hover:shadow-lg hover:shadow-zinc-200/50 transition-all"
    >
      <div className="aspect-square bg-zinc-50 relative overflow-hidden flex items-center justify-center">
        {isImage(file.name) ? (
          ...
          <div className="text-zinc-300 group-hover:scale-110 transition-transform duration-500">
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
          <button 
            onClick={copyLink}
            className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-zinc-900 hover:scale-110 transition-transform active:scale-95"
          >
